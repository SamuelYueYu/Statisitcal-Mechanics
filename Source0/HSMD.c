...
int main(int argc, char *argv[])
{
 ...
 double tvanish; // time needs to vanish
 ...
 double Sum_K,Sum_U,Sum_E,Sum_W,Sum_WLeft,Sum_WRight,Sum_RhoL,Sum_RhoR,Sum_T;
 double Count, pCount;
...
while(1) // relaxation loop
{
  ...
  /*********************************/
   ...
   if(time >= SCT){Jlist(MIN(MIN(tij,tescape), MIN(t_mem,tvanish)));}
   if(n == 999){
  ...
  for(i=0;i<NumberOfParticles;i++)
  {
   if(VanishTime[i] < tvanish)
   {
    tvanish = VanishTime[i];
    VanishID = i;
   }
  }
/*********************************/
if(time+MIN(MIN(tij,tescape), MIN(t_mem,tvanish)) > timewindow) // if time exceeds timewindow, evolve dt/2, break
{
  event = 2;
  if(CellSwitch == 1 && NumberOfCells>NumberOfNeighborCells) tij = MIN(MIN(tij,tescape), MIN(t_mem,tvanish));
...
/************  transfer or collision ********************/
 if(tij == MIN(MIN(tij,tescape), MIN(t_mem,tvanish))) // collision
  {
   event = 0;
   ...
   for(i=0;i<NumberOfParticles;i++)
   {
	CollisionTime[i] -= tij; MemCollisionTime[i] -= tij; VanishTime[i] -= tij;
  ...
  if(CellSwitch == 1 && NumberOfCells > NumberOfNeighborCells) //use Cell list
  {
	  ...
	  if(rim1[i] || rim2[i] || rim3[i] || rim4[i] || rim5[i]){
       	MemCollisionInfo(i); EscapeInfo(i); VanishInfo(i);
    ...
    VanishInfo(I); VanishInfo(J);
  	...
  	if(time > SCT && !n){
	  	...
	  	pCount++; k++; Nleft = 0.; Nright = 0.;
  		for(int i=0;i<NumberOfParticles;i++){NLeftList[i] = -1; NRightList[i] = -1;}
  	}
}// end if collision
 else if(tescape == MIN(MIN(tij,tescape), MIN(t_mem,tvanish))) // if transfer 
 {
 	event = 1;
    tij = tescape;
    ...
    if(CellSwitch == 1 && NumberOfCells>NumberOfNeighborCells){
     ...
     for(i=0;i<NumberOfParticles;i++)
     {
      if(i == EscapeID || onboundx[i]==1 || onboundy[i]==1 || onboundz[i]==1) 
      {
       ...
       EscapeInfo(i); CollisionUpdate(i,celln); MemCollisionInfo(i); VanishInfo(i);
      }// end if particle on boundary
      else{
       EscapeTime[i] -= tij; //only for particles not on boundary
  	  }
  	  if(rim1[i] || rim2[i] || rim3[i] || rim4[i] || rim5[i]){
       	MemCollisionInfo(i); VanishInfo(i);
		for(int j = 0; j<NumberOfParticles; j++){if(j == i || CollisionPartner[j] == i){CollisionInfo(j);}}
 ...
 else if(t_mem == MIN(MIN(tij,tescape), MIN(t_mem,tvanish))){ // membrane collision
	  event = 3;
	  tij = t_mem;
	  time += tij;
  ...
  else if(tvanish == MIN(MIN(tij,tescape), MIN(t_mem,tvanish))){
  	event = 4;
	tij = tvanish;
	time += tij;
 ...
 MFPL = kB*T / (sqrt(2)*M_PI*pLeftVirial*SQR(sigmaB));
 MFPR = kB*T / (sqrt(2)*M_PI*pRightVirial*SQR(sigmaB));
 Kn[0] = MFPL/(D-sigmaB); Kn[2] = MFPR/(D-sigmaB);
 Kn[1] = 2*Kn[0]*Kn[2]/(Kn[0]+Kn[2]); Kn[3] = Kn[1];
 ...
 if(averaged_over){
	...
	yA = nZ2_1+nZ2_2 == 0. ? 0. : nZ2_1/(nZ2_1+nZ2_2);
	yB = nZ2_1+nZ2_2 == 0. ? 0. : nZ2_2/(nZ2_1+nZ2_2);
