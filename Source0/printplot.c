...
void Rho(void){
	...
	nZ2_1 += N_Z2_1/(Lx*Ly*(Z3-Z2-sigmaA)-M_PI*CUBIC(sigmaV)/6); nZ3_1 += N_Z3_1/(Area1*(Lz-Z3+sigmaA));
	
	n0_2 += N_0_2/(Lx*Ly*(Z1-sigmaB)); nZ1_2 += N_Z1_2/(Area2*(Z2-Z1+sigmaB));
	nZ2_2 += N_Z2_2/(Lx*Ly*(Z3-Z2-sigmaB)-M_PI*CUBIC(sigmaV)/6); nZ3_2 += N_Z3_2/(Area2*(Lz-Z3+sigmaB));
...
void JPalpha(void){
	...
	FILE *fpKn; // mean free paths
 	...
 	sprintf(filename,"Kn.txt");
 	fpKn=fopen(filename,"w");
 	fprintf(fpKn,"Kn[0]\tKn[1]\tKn[2]\tKn[3]\n");
 	fprintf(fpKn,"%lf\t%lf\t%lf\t%lf\n",Kn[0],Kn[1],Kn[2],Kn[3]);
 	fclose(fpKn);
