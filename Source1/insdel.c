...
void Insert(double z_small, double z_great, int Identity, double t){
	...
	if(yes && Identity == 1){
		...
		for(int k = 0; k < MAX(N_0_2, N_Z2_2); k++){
			if(n_0_2[k] == i){
				n_0_2[k] = j; break;
			}
			else if(n_Z2_2[k] == i){
    	...
    	for(int k = 0; k<NumberOfParticles; k++){InsdelCollisionUpdate(k, CellTrack[k].WhichCell); MemCollisionInfo(k);}
	}
	else if(yes){
		if(z_small == 0.){
...
void Delete(double z_small, double z_great, int Identity, double t){
	...
	if(yes && Identity == 1){
		...
		if(i != j){
   		...
   		for(int l = 0; l<NumberOfParticles; l++){InsdelCollisionUpdate(l, CellTrack[l].WhichCell); MemCollisionInfo(l);}
